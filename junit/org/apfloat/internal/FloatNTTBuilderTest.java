...
public class FloatNTTBuilderTest
    extends FloatTestCase
{
    public FloatNTTBuilderTest(String methodName)
    {
        super(methodName);
    }
    ...
    public static TestSuite suite()
    {
        ...
        suite.addTest(new FloatNTTBuilderTest("testCreate"));
    ...
    {
        NTTBuilder nttBuilder = new FloatNTTBuilder();
        ApfloatContext ctx = ApfloatContext.getContext();
        int cacheSize = ctx.getCacheL1Size() / 4;
        long maxMemoryBlockSize = ctx.getMaxMemoryBlockSize() / 4;

        assertTrue("Fits in cache", nttBuilder.createNTT(cacheSize / 2) instanceof FloatTableFNTStrategy);
