...
public class FloatCarryCRTStrategyTest
    extends FloatTestCase
    implements FloatModConstants, FloatRadixConstants
{
    public FloatCarryCRTStrategyTest(String methodName)
    {
        super(methodName);
    }
    ...
    public static TestSuite suite()
    {
        ...
        suite.addTest(new FloatCarryCRTStrategyTest("testFullLength"));
        suite.addTest(new FloatCarryCRTStrategyTest("testTruncatedLength"));
        suite.addTest(new FloatCarryCRTStrategyTest("testBigFullLength"));
        suite.addTest(new FloatCarryCRTStrategyTest("testBigFullLengthParallel"));
    ...
    public static void testFullLength()
    {
        for (int radix = Character.MIN_RADIX; radix <= Character.MAX_RADIX; radix++)
        {
            ...
            DataStorage src0 = createDataStorage(new float[] { 0, value.mod(m0).floatValue(), value.mod(m0).floatValue(), 0 }),
                        src1 = createDataStorage(new float[] { 0, value.mod(m1).floatValue(), value.mod(m1).floatValue(), 0 }),
                        src2 = createDataStorage(new float[] { 0, value.mod(m2).floatValue(), value.mod(m2).floatValue(), 0 });

            StepCarryCRTStrategy crt = new StepCarryCRTStrategy(radix);

            DataStorage dst = crt.carryCRT(src0, src1, src2, 4);

            check("normal", radix, new float[] {(float) 1, (float) 3, (float) 5, (float) 3 }, dst);
        }
    }
    ...
    public static void testTruncatedLength()
    {
        for (int radix = Character.MIN_RADIX; radix <= Character.MAX_RADIX; radix++)
        {
            ...
            DataStorage src0 = createDataStorage(new float[] { 0, value.mod(m0).floatValue(), (float) 1, 0 }),
                        src1 = createDataStorage(new float[] { 0, value.mod(m1).floatValue(), (float) 1, 0 }),
                        src2 = createDataStorage(new float[] { 0, value.mod(m2).floatValue(), (float) 1, 0 });

            StepCarryCRTStrategy crt = new StepCarryCRTStrategy(radix);

            DataStorage dst = crt.carryCRT(src0, src1, src2, 1);

            check("normal", radix, new float[] {(float) 1 }, dst);
        }
    }
    ...
    private static void runBig()
    {
        for (int radix = Character.MIN_RADIX; radix <= Character.MAX_RADIX; radix++)
        {
            ...
            float[] data0 = new float[SIZE],
                      ...
                      expected = new float[SIZE];

            float value0 = value.mod(m0).floatValue(),
                    value1 = value.mod(m1).floatValue(),
                    value2 = value.mod(m2).floatValue();
            ...
            for (int i = 1; i < SIZE - 1; i++)
            {
                ...
                data1[i] = value1;
                data2[i] = value2;
                expected[i] = BASE[radix] - (float) 1;
            }
            expected[0] = BASE[radix] - (float) 1;
            expected[1] = BASE[radix] - (float) 2;
            expected[SIZE - 2] = 0;
            expected[SIZE - 1] = (float) 1;
