...
public class DoubleFactor3NTTStrategyTest
    extends DoubleNTTStrategyTestCase
{
    public DoubleFactor3NTTStrategyTest(String methodName)
    {
        super(methodName);
    }
    ...
    public static TestSuite suite()
    {
        ...
        suite.addTest(new DoubleFactor3NTTStrategyTest("testForward"));
        suite.addTest(new DoubleFactor3NTTStrategyTest("testRoundTrip"));
        suite.addTest(new DoubleFactor3NTTStrategyTest("testRoundTripParallel"));
        suite.addTest(new DoubleFactor3NTTStrategyTest("testRoundTrip2"));
        suite.addTest(new DoubleFactor3NTTStrategyTest("testRoundTrip2Parallel"));
    ...
    public static void testForward()
    {
        for (int modulus = 0; modulus < 3; modulus++)
        {
            ...
            NTTStrategy nttStrategy = new Factor3NTTStrategy(new DoubleTableFNTStrategy());
    ...
    {
        int size = (int) Math.min(3 * 1048576, DoubleModConstants.MAX_TRANSFORM_LENGTH);       // Will use six-step transform
        DataStorage dataStorage = createDataStorage(size + 5).subsequence(5, size);
    ...
    private static void runRoundTrip2()
    {
        ...
        for (int modulus = 0; modulus < 3; modulus++)
        {
            Factor3NTTStrategy nttStrategy = new Factor3NTTStrategy(new DoubleTableFNTStrategy());
