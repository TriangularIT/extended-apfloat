...
public class FloatFactor3NTTStrategyTest
    extends FloatNTTStrategyTestCase
{
    public FloatFactor3NTTStrategyTest(String methodName)
    {
        super(methodName);
    }
    ...
    public static TestSuite suite()
    {
        ...
        suite.addTest(new FloatFactor3NTTStrategyTest("testForward"));
        suite.addTest(new FloatFactor3NTTStrategyTest("testRoundTrip"));
        suite.addTest(new FloatFactor3NTTStrategyTest("testRoundTripParallel"));
        suite.addTest(new FloatFactor3NTTStrategyTest("testRoundTrip2"));
        suite.addTest(new FloatFactor3NTTStrategyTest("testRoundTrip2Parallel"));
    ...
    public static void testForward()
    {
        for (int modulus = 0; modulus < 3; modulus++)
        {
            ...
            NTTStrategy nttStrategy = new Factor3NTTStrategy(new FloatTableFNTStrategy());
    ...
    {
        int size = (int) Math.min(3 * 1048576, FloatModConstants.MAX_TRANSFORM_LENGTH);       // Will use six-step transform
        DataStorage dataStorage = createDataStorage(size + 5).subsequence(5, size);
    ...
    private static void runRoundTrip2()
    {
        ...
        for (int modulus = 0; modulus < 3; modulus++)
        {
            Factor3NTTStrategy nttStrategy = new Factor3NTTStrategy(new FloatTableFNTStrategy());
