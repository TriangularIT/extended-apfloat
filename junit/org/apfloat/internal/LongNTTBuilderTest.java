...
public class LongNTTBuilderTest
    extends LongTestCase
{
    public LongNTTBuilderTest(String methodName)
    {
        super(methodName);
    }
    ...
    public static TestSuite suite()
    {
        ...
        suite.addTest(new LongNTTBuilderTest("testCreate"));
    ...
    {
        NTTBuilder nttBuilder = new LongNTTBuilder();
        ApfloatContext ctx = ApfloatContext.getContext();
        int cacheSize = ctx.getCacheL1Size() / 8;
        long maxMemoryBlockSize = ctx.getMaxMemoryBlockSize() / 8;

        assertTrue("Fits in cache", nttBuilder.createNTT(cacheSize / 2) instanceof LongTableFNTStrategy);
